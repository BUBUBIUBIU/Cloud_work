...
const properties = {
    ...
    bounds: 'bounds',
    name: 'phn_name',
    high_blood_pressure: 'est_ppl_18yrs_plus_hi_blood_pressure_2014_15_asr_100',
    overweight: 'est_ppl_18yrs_plus_ovrwht_2014_15_asr_100',
    obese: 'est_ppl_18yrs_plus_obese_2014_15_asr_100',
    high_psychological_distress: 'est_ppl_18yrs_plus_hi_psyc_strs_k10_scal_2014_15_asr_100'
};

const JSON_FILE = 'data/PHIDU.json';
...
function loadTweets() {
    tweet_obj = JSON.parse(get('http://qwe:qwe@172.26.37.179:5984/glutton_collected/_design/sentiment_analysis/_view/sentiment_analysis?group=true'));
...
function getNormalisedValue(f, disp) {
    ...
    // Flip all but high_blood_pressure.
    if (disp != "high_blood_pressure") {
        norm = 1 - norm;
    }
...
function generateInfoWindowContent(f) {
    ...
    high_blood_pressure = f.getProperty(properties.high_blood_pressure).toFixed(2)
    high_psychological_distress = f.getProperty(properties.high_psychological_distress).toFixed(2)
    ...
    const name = f.getProperty(properties.name)
    code = f.getProperty(properties.id)
    ...
    str += "<h4>" + name + "</h4>"
    str += "<table>"
    str += "<tr><td> High blood pressure per 100 people: </td><td>" + high_blood_pressure + "</td></tr>"
    str += "<tr><td> High psychological distress per 100 people: </td><td>" + high_psychological_distress + "</td></tr>"
...
function refreshDataNew() {
    ...
    if (document.checks.high_blood_pressure.checked) {
        checkCount += 1
    }

    if (document.checks.high_psychological_distress.checked) {
        checkCount += 1
    }
    ...
    if (document.checks.high_blood_pressure.checked) {
        checkCount += 1
        disp.push("high_blood_pressure")
    }

    if (document.checks.high_psychological_distress.checked) {
        checkCount += 1
        disp.push("high_psychological_distress")
    }
